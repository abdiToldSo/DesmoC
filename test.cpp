...
void DrawInputBox(char* name, bool mouseOnText, int letterCount, int frameCounter); 
void HandleInputBox(char* name, bool &mouseOnText, int &letterCount, int &frameCounter); 
...
int main() {
	...
	while (!WindowShouldClose()) {
		...
		//HandleCameraInput();
		if (IsAnyKeyPressed()) test.HandleInput(name);
		...
		BeginDrawing();
			...
			//test.DrawPoints();
			test.DrawPlot();
			DrawText(inputText, (GetScreenWidth()/2.0f - MeasureText(inputText, 20) / 2), 70, 20, GRAY);
			//DrawText(inputText, (GetScreenWidth() / 2), 70, 20, GRAY);
			DrawInputBox(name, mouseOnText, letterCount, frameCounter);
			//DrawText(TextFormat("INPUT CHARS: %i/%i", letterCount, MAX_INPUT_CHARS), , 100, 20, DARKGRAY);
			DrawText(TextFormat("INPUT CHARS: %i/%i", letterCount, MAX_INPUT_CHARS), 
					GetScreenWidth()/2.0f - MeasureText(TextFormat("INPUT CHARS: %i/%i", letterCount, MAX_INPUT_CHARS), 20)/2.0f
					,
					100, 20, DARKGRAY);
...
void DrawInputBox(char* name, bool mouseOnText, int letterCount, int frameCounter) {
	...
	if (mouseOnText) {
		...
		else DrawText("Press BACKSPACE to delete chars...", 230, 110, 20, GRAY);
	}
}
