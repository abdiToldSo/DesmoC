...
void Plane::DrawPlane() {
  ...
  //for (; i < GetScreenWidth(); i += GetScreenWidth()/22) {
          //if (!( (i < GetScreenWidth()/2.0) +10 && (i > GetScreenWidth()/2.0)
  - 10)) {
          //if ( !(GetScreenWidth()/2.0 - 10.0 <= i && i <= GetScreenWidth()/2.0
  + 10.0) ) { DrawLine(i, 0, i, GetScreenHeight(), PURPLE );
          //}
  }
  ...
  //for (; i < GetScreenWidth(); i += GetScreenWidth()/22) {
          //if (!( (i < GetScreenWidth()/2.0) +10 && (i > GetScreenWidth()/2.0)
  - 10)) { if ( !(GetScreenHeight()/2.0f - 5.0 <= i && i <=
  GetScreenHeight()/2.0f + 5.0) ) { DrawLine(0, i+1, GetScreenWidth(), i+1,
  PURPLE );
          }
  }
  */
...
void Plane::DrawPoints() {
  for (int i = 0; i < 23; i++) {
    ...
    DrawCircle(
        // GetScreenWidth()/2.0f + 0.0f + points[i].x*(GetScreenWidth()/23.0f
        // +0.0f) - 2*points[i].x, GetScreenWidth()/2.0f + 0.0f +
        // points[i].x*(GetScreenWidth()/23.0f +0.0f) - 2*points[i].x,
        GetScreenWidth() / 2.0f + 0.0f +
            points[i].x * (GetScreenWidth() / 23.0f + 0.0f),
        // GetScreenHeight()/2.0f + points[i].y*-22.0f,
        GetScreenHeight() / 2.0f + 0.0f +
            points[i].y * -(GetScreenHeight() / 23.0f + 0.0f),
        // GetScreenHeight()/2.0f + 0.0f + points[i].y*-(GetScreenHeight()/23.0f
        // +0.0f), points[i].x + GetScreenWidth()/2.0f, points[i].y +
        // GetScreenHeight()/2.0f,
        8.0f, RED);
    // DrawLine(points[i].x*(GetScreenWidth()/23.0f)+GetScreenWidth()/2.0f, 0,
    // points[i].x*(GetScreenWidth()/23.0f)+GetScreenWidth()/2.0f,
    // GetScreenHeight(), PURPLE); DrawLine(0, GetScreenHeight()/2.0f + 0.0f +
    // points[i].y*-(GetScreenHeight()/23.0f +0.0f), GetScreenWidth(),
    // GetScreenHeight()/2.0f + 0.0f + points[i].y*-(GetScreenHeight()/23.0f
    // +0.0f), PURPLE );
  }
}
void Plane::DrawPlot() {
  ...
  for (int i = 0; i < 23; i++) {
    ...
    /*
    pointsInterleaved[3*i] = (Vector2) {GetScreenWidth()/2.0f + 0.0f +
    points[i].x*(GetScreenWidth()/23.0f +0.0f), GetScreenHeight()/2.0f + 0.0f +
    points[i].y*-(GetScreenHeight()/23.0f +0.0f),}; pointsInterleaved[3*i + 1] =
    (Vector2) {GetScreenWidth()/2.0f + 0.0f +
    points[i+1].x*(GetScreenWidth()/23.0f +0.0f), GetScreenHeight()/2.0f + 0.0f
    + points[i+1].y*-(GetScreenHeight()/23.0f +0.0f),}; pointsInterleaved[3*i +
    2] = (Vector2) {GetScreenWidth()/2.0f + 0.0f +
    points[i+2].x*(GetScreenWidth()/23.0f +0.0f), GetScreenHeight()/2.0f + 0.0f
    + points[i+2].y*-(GetScreenHeight()/23.0f +0.0f),}; pointsInterleaved[3*i +
    1] = points[i]; pointsInterleaved[3*i + 2] = points[i];
    */
